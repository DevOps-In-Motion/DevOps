...
// deduplicating job requests and pushing them onto a queue (such as Kafka).
type Service struct {
	queue   JobQueue
...
// and a DistributedLocker (e.g., Redis, DynamoDB, etc.).
func New(queue JobQueue, locker DistributedLocker, lockTTL time.Duration) (*Service, error) {
	if queue == nil {
...
// ScheduleJob applies a distributed lock for the tenant/job combination and,
// if successful, pushes the job onto Kafka.
func (s *Service) ScheduleJob(ctx context.Context, req *mcpschedulerv1.CreateJobRequest) (*mcpschedulerv1.CreateJobResponse, error) {
	...
	// Derive a stable key that represents "same user + same automation".
	// Here we use organization_id + job_type + hash(prompt + parameters).
...
// hashJobSignature creates a deterministic hash of the prompt and parameters
// so we can identify "same automation" retries.
func hashJobSignature(req *mcpschedulerv1.CreateJobRequest) string {
	h := sha256.New()
	h.Write([]byte(req.GetPrompt()))
	h.Write([]byte(req.GetJobType()))

	// Parameters map iteration order is random; normalize by sorting keys.
	if len(req.GetParameters()) > 0 {
		...
		// Simple insertion sort since map sizes are typically small.
